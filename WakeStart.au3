...
#AutoIt3Wrapper_Res_Fileversion=0.3.0.28
...
While 1
   ...
   history ("Choosed option *Ready to start install server* " & $Button_9)
	...
	history ("Deleted old files " & $inifile & ", " & $resultini)
	  ...
	  IniWrite($inifile, "Time", "ClientPause", $ClientPause )
		...
		history ("Starting process Ч " & $ScriptFolder & "\" & $WakePrepare & " Server")
   ...
   history ("Choosed option *Ready to finish client setup* " & $Button_5)
	  ...
	  EndIf
		...
		history ("Main GUI destroyed Ч " & $destr)
		PauseTime($pausetime)
		history ("Starting process Ч " & $ScriptFolder & "\" & $WakePrepare & " Client")
