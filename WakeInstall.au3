...
#AutoIt3Wrapper_Res_Fileversion=0.3.0.20
...
$F_inst=GUICtrlCreateCheckbox("Fresh install ", 100, 80, 120, 20)
...
While 1
	...
	_ArrayAdd($FilesInstallArray, FileInstall("WakeUninstall.exe", $ScriptFolder & "\" & $WakeUninstall))
	_ArrayAdd($FilesInstallArray, FileInstall("WakePrepare.exe", $ScriptFolder & "\" & $WakePrepare))
	_ArrayAdd($FilesInstallArray, FileInstall("WakeServer.exe", $ScriptFolder & "\" & $WakeServer))
	_ArrayAdd($FilesInstallArray, FileInstall("WakeClient.exe", $ScriptFolder & "\" & $WakeClient))
	_ArrayAdd($FilesInstallArray, FileInstall("WakeStart.exe", $ScriptFolder & "\" & $WakeStart))
	_ArrayAdd($FilesInstallArray, FileInstall("WakeDaemon.exe", $ScriptFolder & "\" & $WakeDaemon))
	_ArrayAdd($FilesInstallArray, FileMove(@TempDir & "\" & $helpfile, $ScriptFolder & "\" & $helpfile,1))
