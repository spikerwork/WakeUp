...
;Global $current_run="Current run #" & $run
   ...
   Func history ($post)
	  ...
	  FileWrite($file, $time & " Script (" & @ScriptName & ") Ч - Ч " & $post & @CRLF)
	  FileClose($file)
			...
			If $cpu_need==1 Then
			   ...
			   If $AverageLoadCPU == "" Then $AverageLoadCPU=100
				  ...
				  If $AverageLoadCPU > $cpu_percent_need Then
					 ...
					 history ("Average CPU Load is too high Ч " & $AverageLoadCPU & "%. Default is " & $cpu_percent_need & "%")
					 $worktime=$worktime+$time
					 $time=0
				  
				  Else
				  
					 history ("Average CPU Load is low than Ч " & $cpu_percent_need & "%")  
			...
			If $hdd_need==1 Then
			   ...
			   history ("HDD bytes sent (DiskBytesPerSec): " &  $hdd_bytes)
				  ...
				  If $AverageLoadHDD > $hdd_percent_need Then
					 ...
					 history ("Average HDD Load is too high Ч " & $AverageLoadHDD & "%. Default is " & $hdd_percent_need & "%")
					 $worktime=$worktime+$time
					 $time=0
					 
				  Else
					 
					 history ("Average HDD Load is low than Ч " & $hdd_percent_need & "%")
